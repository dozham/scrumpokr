...
export function ResultsSummary({ votes, participants }: Props) {
  ...
  return (
    <div className="bg-[#fff8b3] rounded-xl px-4 py-3 flex items-center gap-6 text-sm flex-wrap">
      <span className="text-[#2a2380]/70">Avg: <strong className="text-[#2a2380]">{avg}</strong></span>
      <span className="text-[#2a2380]/70">Min: <strong className="text-[#2a2380]">{String(min)}</strong></span>
      <span className="text-[#2a2380]/70">Max: <strong className="text-[#2a2380]">{String(max)}</strong></span>
      {consensus !== undefined && (
        <span className="text-[#7ab840] font-semibold ml-auto">
