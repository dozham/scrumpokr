...
export function ResultsSummary({
  ...
  const voterIds = participants
    .filter((p) => p.role === "voter")
    .map((p) => p.id);
  const allVoterCards = voterIds
    .map((id) => votes[id])
    .filter((c): c is Card => c !== undefined);
  const numeric = allVoterCards.filter(
  ...
  const naturalConsensus =
    allVoterCards.length === voterIds.length &&
    voterIds.length > 0 &&
    new Set(allVoterCards.map(String)).size === 1
      ? allVoterCards[0]
      : undefined;

  const uniqueVotedValues = [
    ...new Map(allVoterCards.map((c) => [String(c), c])).values(),
  ];
    ...
    <div className="bg-sky-50 dark:bg-gray-800 border border-sky-200 dark:border-transparent rounded-xl px-4 py-3 text-sm">
      <div className="flex items-center gap-6 flex-wrap">
        ...
        <span className="text-sky-600 dark:text-gray-400">
          Min: <strong className="text-slate-900 dark:text-white">{min}</strong>
        </span>
        <span className="text-sky-600 dark:text-gray-400">
          Max: <strong className="text-slate-900 dark:text-white">{max}</strong>
        </span>
        <span className="text-sky-600 dark:text-gray-400">
          Most Voted:{" "}
